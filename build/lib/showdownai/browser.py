...
class Selenium():
    ...
    def __init__(self, driver_path, url=BASE_URL, timer_on=False, proxy=False, browser='chrome'):
        ...
        self.driver = webdriver.Chrome(executable_path=self.driver_path+'/chromedriver', chrome_options = options)
        if browser == "firefox":
    ...
    def get_my_primary(self):
        img = self.driver.find_elements_by_css_selector(".innerbattle img")[1]
    ...
    def get_opp_primary(self):
        img = self.driver.find_elements_by_css_selector(".innerbattle img")[0]
        text = img.get_attribute('src')
        poke = text.split("/")[-1]
        poke = poke[:-4]
        return poke
    ...
    def wait_for_move(self):
        ...
        while move_exists == False:
            ...
            if self.check_exists_by_css_selector(".button[name='saveReplay']") or self.check_exists_by_css_selector(".button[name='instantReplay']"):
                self.chat("gg")
                self.driver.save_screenshot("over.png")
