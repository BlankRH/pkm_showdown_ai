...
from agent import OptimisticMinimaxAgent, BaselineAgent
...
class Showdown():
    def __init__(self, team_text, agent, username, pokedata, password=None,
        ...
        #print(arch[0])
        self.lib_dir = Path(lib_dir + 'mac')
        self.selenium = Selenium(proxy=proxy, browser=browser, driver_path = self.lib_dir)
    ...
    def test(self):
        user_folder = Path(".") / self.username / self.agent
        print(user_folder)
...
def main():
    ...
    argparser.add_argument('--team', default='mamo')
    argparser.add_argument('--username', default='hubbbb')
    argparser.add_argument('--password', default='VouchSuperBohos+2')
    ...
    showdown = Showdown(
        team_text,
        #OptimisticMinimaxAgent(2, pokedata),
        BaselineAgent(pokedata),
        args.username,
        pokedata,
