...
#for human agent, create an legal action and print out
...
class MinimaxAgent(Agent):
    ...
    def get_action(self, state, who, log=True):
        ...
        elapsed = end - start#time used -> log_file
...
class BaselineAgent(Agent):
    def __init__(self, pokedata, use_cache=True, log_file=None):
        self.simulator = Simulator(pokedata)
        self.cache = {}
        self.hit_count = 0
        self.prune_count = 0
    ...
    def get_action(self, state, who=0, log=True):
        start = time.time()
        maxv = -100000
        epsilon = 0.9
        my_legal_actions = state.get_legal_actions(who)
        ...
        if np.random.uniform() > epsilon:
            best_action = choice(my_legal_actions)
        else:
