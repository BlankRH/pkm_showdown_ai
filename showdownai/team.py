...
class Pokemon():
    ...
    def mega_evolve(self, pokedata, log=False):
        if self.can_evolve():
            ...
            moveset = SmogonMoveset(self.moveset.name, None, ability, self.moveset.evs, self.moveset.nature, self.moveset.moves, tag=self.moveset.tag)
            status = self.status
...
if __name__ == "__main__":
    import json
    with open("data/poke3.json") as f:
    ...
    moveset = SmogonMoveset.from_dict(poke.movesets[0])
